...
public:
    ...
    CUpdateCallback(const SZUpdateItem *items_, UInt32 count, const char *password_)
        : items(items_),
          itemCount(count),
          password(password_ ? password_ : ""),
          hasPassword(password_ && password_[0] != '\0') {}
...
{
    *passwordIsDefined = hasPassword ? 1 : 0;
    if (!hasPassword) {
        *password = nullptr;
        return S_OK;
    }

    UString uPassword = UTF8ToUString(this->password.c_str());
    ...
    try {
        ...
        if (options) {
            ...
            if (setProps) {
                ...
                // Password
                UString passwordName;
                if (options->password && options->password[0] != '\0') {
                    passwordName = L"p";
                    names.push_back(passwordName);
                    UString passwordVal = UTF8ToUString(options->password);
        ...
        CUpdateCallback *callbackSpec = new CUpdateCallback(
            items,
            item_count,
            options ? options->password : nullptr
        );
