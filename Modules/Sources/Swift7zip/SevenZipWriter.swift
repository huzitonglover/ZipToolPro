        ...
        try withExtendedLifetime(dataRefs) {
            ...
            try withArrayOfCStrings(archivePaths) { archivePathPtrs in
                try withArrayOfCStrings(diskPaths) { diskPathPtrs in
                    ...
                    let formatStr = options.rawFormatName ?? options.format.rawValue
                    let methodStr = options.method?.rawValue
                    let passwordStr = options.password
                    ...
                    try formatStr.withCString { fmtPtr in
                        ...
                        let callBridge = { (methodPtr: UnsafePointer<CChar>?, passwordPtr: UnsafePointer<CChar>?) throws in
                            cOptions.method = methodPtr
                            cOptions.password = passwordPtr
                        ...
                        let callWithPassword = { (methodPtr: UnsafePointer<CChar>?) throws in
                            if let passwordStr, !passwordStr.isEmpty {
                                try passwordStr.withCString { passwordPtr in
                                    try callBridge(methodPtr, passwordPtr)
                                }
                            } else {
                                try callBridge(methodPtr, nil)
                            }
                        }

                        if let m = methodStr {
                            try m.withCString { mPtr in
                                try callWithPassword(mPtr)
                            }
                        } else {
                            try callWithPassword(nil)
                        }
                    }
                }
            }
