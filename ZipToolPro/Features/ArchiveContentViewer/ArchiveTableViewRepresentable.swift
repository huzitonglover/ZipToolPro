                ...
                if isParent {
                    ...
                    if let cached = item.icon {
                        ...
                        if item.type == .directory {
                            ...
                            computed = NSWorkspace.shared.icon(for: contentType)
                        }
                        item.icon = computed
                        image = computed
                    }
...
//                    textField.leadingAnchor.constraint(equalTo: imageView.trailingAnchor, constant: 5).isActive = true
            ...
            } else if columnIdentifier == ArchiveViewerColumn.uncompressedSize.identifier {
                let displaySize = displaySize(for: item)
                let sizeAsString = displaySize.map { SystemHelper.shared.format(bytes: $0) } ?? ""
        ...
        private func displaySize(for item: ArchiveItem) -> Int? {
            if item.type == .directory || item.type == .virtual || item.type == .root {
                let total = recursiveSize(for: item)
        ...
        private func recursiveSize(for item: ArchiveItem) -> Int {
            guard let children = item.children else { return 0 }
            return children.reduce(0) { total, childID in
                guard let child = parent.archiveState.entries[childID] else {
                    return total
                }
                if child.type == .directory || child.type == .virtual || child.type == .root {
                    return total + recursiveSize(for: child)
                }
                if child.uncompressedSize > 0 {
                    return total + child.uncompressedSize
                }
                if child.compressedSize > 0 {
                    return total + child.compressedSize
                }
                return total
            }
        }
